...
// <Line>  ::= { <blank line> | '#" <arbitrary text> | <Rule> | <Sitemap> } <LineEnd>
...
// <CrawlDelay> ::= "Crawl-delay:" <integer>
...
// a User-Agent line whose value contains the name of the robot
...
// in seconds before recrawling.  It's ambiguous whether that's the delay
...
private:
    struct Node {
        ...
        Node() : domain(""), rules(nullptr), prev(nullptr), next(nullptr) {};
    };
public:

    RobotsManager() : capacity(ROBOTS_CACHE_SIZE) {
        head = new Node();
        tail = new Node();
        head->next = tail;
        tail->prev = head;
    ...
    CrawlStatus checkStatus(const string& domain, const string& url) {
        // check if domain in cache first
        Node* node = fetchAndParse(domain);
        ...
        return node->rules->UrlAllowed(reinterpret_cast<const Utf8*>(USER_AGENT), reinterpret_cast<const Utf8*>(url.data()), nullptr) ? CrawlStatus::ALLOWED : CrawlStatus::DISALLOWED;
    }
